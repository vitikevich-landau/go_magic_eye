...
| `Cycle` | не-nil ⇒ узел-ссылка на уже показанный узел (`⟲` цикл / `≡` разделение) |
| `Shared` | оригинал `Cycle` — НЕ предок: разделяемая ссылка `≡` (ромб/DAG), не цикл |
...
| pointer | один ребёнок «➤ цель» — живой `Elem()`; если адрес+тип уже показаны — узел-ссылка (`⟲` — оригинал среди предков, иначе `≡`) |
...
| `seen` | `map[{addr,type}]*Node` — учёт показанных адресов: дубли не плодятся, повторы получают `⟲`/`≡` |
...
строка 2      путь-крошки узла под курсором (середина сжимается первой)
строки 3…H-2  дерево (≈2/5 ширины) │ «Гримуар» (детали узла)
строка H-1    гид по клавишам / строка поиска / статус-сообщение
```

У краёв прокрутки дерева — маркеры «↑ ещё N выше / ↓ ещё M ниже». `Esc`
шагает наружу по слою (помощь → панель → фокус), выходит только двойной
`Esc` подряд; `q` выходит сразу.
...
- `Color`, `ASCII` — глобальные выключатели (ставятся при каждом входе в
  публичный API: опции `WithColor`/`WithASCII` сильнее `EYE_COLOR`/`EYE_ASCII`).
